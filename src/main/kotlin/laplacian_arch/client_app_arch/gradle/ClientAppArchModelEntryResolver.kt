...
class ClientAppArchModelEntryResolver: ModelEntryResolver {
    ...
    override fun resolve(key: String, model: Map<String, RecordList>, context: ExecutionContext): Any? {
        return when (key) {
            "pages" -> PageList(
                model.getList<Record>("pages").map{ PageRecord(it, context.currentModel) },
                context.currentModel
            )
            "clients" -> ClientList(
                model.getList<Record>("clients").map{ ClientRecord(it, context.currentModel) },
                context.currentModel
            )
            "widgets" -> WidgetList(
                model.getList<Record>("widgets").map{ WidgetRecord(it, context.currentModel) },
                context.currentModel
            )
            "features" -> FeatureList(
                model.getList<Record>("features").map{ FeatureRecord(it, context.currentModel) },
                context.currentModel
            )
